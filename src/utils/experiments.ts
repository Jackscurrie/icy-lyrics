 ...
 *     default: true,
 *     pageClass: "Exp_MyExperiment",   // optional — toggled on #IcyLyricsPage
 *     rebuildsNowBar: true,            // optional — see below
 *   }
 *
 * `pageClass` is the preferred way to implement one: write the new look under
 * `#IcyLyricsPage.Exp_Foo` and keep the old one under
 * `#IcyLyricsPage:not(.Exp_Foo)`, and the toggle costs nothing at runtime.
 *
...
export type Experiment = {
  ...
  description: string;
  default: boolean;
  /** Class toggled on `#IcyLyricsPage` while the experiment is enabled. */
  pageClass?: string;
