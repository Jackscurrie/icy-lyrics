...
export const UI_STATE_KEY = "IcyLyrics:uiState";
...
function migrateUiStateKeys(blob: Record<string, any>): Record<string, any> {
  ...
  for (const retiredKey of ["fromVersion", "previousVersion", "previous-version"]) {
    if (retiredKey in blob) {
      delete blob[retiredKey];
      changed = true;
    }
  }
