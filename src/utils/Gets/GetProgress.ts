...
import { getLyricsPlaybackClockPosition } from "../PlaybackClock.ts";
...
export default function GetProgress() {
  ...
  if (!Spicetify.Player.isPlaying()) {
    // Static when paused. Keep the same perceptual forward lead as the playing
    // branch so pausing does not jump word, line-transition, or outro frames
    // backward by 100ms.
    return normalizeProgress(getLyricsPlaybackClockPosition(Position, playbackOffset), false);
  }

  const FinalPosition = Position + deltaTime;
  return normalizeProgress(getLyricsPlaybackClockPosition(FinalPosition, playbackOffset), true);
}

/**
 * Unshifted audio position for presentation that must end at the actual track
 * handoff. This shares the same synchronized anchor as the lyric clock but
 * deliberately excludes lyric lead and the user playback offset.
 */
export function GetRawProgress(): number {
  if (SpotifyPlayer.GetContentType() !== "track") {
    return clampToTrack(Spicetify.Player.getProgress());
  }

  if (!syncedPosition) {
    const fallback = _DEPRECATED___GetProgress();
    return clampToTrack(typeof fallback === "number" ? fallback : 0);
  }

  const { StartedSyncAt, Position } = syncedPosition;
  const rawPosition = Spicetify.Player.isPlaying()
    ? Position + Math.max(0, Date.now() - StartedSyncAt)
    : Position;
  return clampToTrack(rawPosition);
}
