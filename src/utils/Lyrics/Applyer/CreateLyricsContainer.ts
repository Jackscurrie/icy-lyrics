...
const CreateLyricsContainer = (): LyricsContainerReturnObject => {
  ...
  Container.classList.add("IcyLyricsScrollContainer");
  ...
  const runResize = () => {
    // A fullscreen view FLIP has already committed its final layout and is now
    // compositor-only. ResizeObserver fires for that commit, but recalculating
    // SimpleBar on the next frame would force layout in the middle of the
    // animation. Keep the request coalesced until the transition class clears.
    const page = Container.closest<HTMLElement>("#IcyLyricsPage");
    if (
      page?.classList.contains("FullscreenViewTransitioning") ||
      page?.classList.contains("frame_F_Exit")
    ) {
      resizeRAF = requestAnimationFrame(runResize);
      return;
    }
