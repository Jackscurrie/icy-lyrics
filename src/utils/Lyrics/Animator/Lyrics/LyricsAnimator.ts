...
import { BlurMultiplier, getLyricsAnimationPosition } from "../Shared.ts";
...
export function findActiveElement(currentTime: number): any {
  const ProcessedPosition = getLyricsAnimationPosition(currentTime, $simpleLyricsMode.get());
...
export function Animate(position: number): void {
  const ProcessedPosition = getLyricsAnimationPosition(position, $simpleLyricsMode.get());
