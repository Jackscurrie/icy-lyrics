...
import { normalizeLyricsSchema } from "./schema.ts";

// Constants
const RomajiConverter = new Kuroshiro();
const RomajiPromise = RomajiConverter.init(KuromojiAnalyzer);
// The package runtime is loaded lazily from a remote module. Attach a rejection
// handler immediately so offline startup/test environments do not produce an
// unhandled rejection; callers that actually need Japanese still await the
// original promise and receive the failure normally.
void RomajiPromise.catch(() => undefined);
