import { $lyricsContainerExists, $minimalLyricsMode, $simpleLyricsMode } from "../stores.ts";
 ...
 * function is ownership-bound, so an older preview cannot clear a newer one.
 */
...
const LyricsInterval = () => {
  ...
  if ($lyricsContainerExists.get()) {
    ...
    // from the same playback clock even when Simplebar is absent or the
    // virtualizer has temporarily handed its elements to the focus stage.
    Global.Event.evoke(
      "lyrics:frame",
      getLyricsAnimationPosition(progress, $simpleLyricsMode.get()),
      rawProgress
    );
  }
