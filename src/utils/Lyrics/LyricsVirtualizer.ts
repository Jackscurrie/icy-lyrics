...
import { FullscreenDeferredWorkGate } from "../FullscreenPresentation.ts";
...
class LyricsVirtualizer {
  ...
  // A stable, non-animator-owned carrier sits between each positioning wrapper
  // and lyric line. Fullscreen view FLIPs animate this carrier so the renderer's
  // own translate/scale/word transforms remain completely untouched.
  private _carriers: (HTMLElement | null)[] = [];
  ...
  // Fullscreen lyrics focus renders the animator-owned line elements in a
  // viewport-sized stage. The wrappers stay under TanStack's ownership so we
  // can restore the same elements (and their animator state) without cloning
  // or rebuilding the lyrics tree when focus mode closes.
  private _presentationStage: HTMLElement | null = null;
  private _presentationIndices = new Set<number>();
  private _presentationRemeasureGate = new FullscreenDeferredWorkGate();
  ...
  private _remeasureVisible(): void {
    ...
    for (const idx of this._mountedIndices) {
      if (this._presentationIndices.has(idx)) continue;
  ...
  public setPresentationMode(active: boolean, deferRemeasure = false): void {
    if (active) {
      if (this._presentationStage?.isConnected) return;
      // A rapid reversal back into focus mode makes a deferred list remeasure
      // obsolete. The wrappers remain pinned under TanStack and will be
      // measured when presentation mode is eventually left again.
      this._presentationRemeasureGate.cancel();
      const host = this._scrollEl?.closest<HTMLElement>(".LyricsContent");
      if (!host) return;
      const stage = document.createElement("div");
      stage.className = "FullscreenLyricsFocusStage";
      stage.setAttribute("aria-live", "off");
      host.appendChild(stage);
      this._presentationStage = stage;
      host.closest<HTMLElement>("#IcyLyricsPage")?.classList.add("FullscreenLyricsPresenting");
      return;
    }

    if (deferRemeasure) {
      this.deferPresentationRemeasure();
    } else {
      this._presentationRemeasureGate.cancel();
    }

    this._restorePresentationElements();
    this._presentationStage?.remove();
    this._presentationStage = null;
    this._scrollEl
      ?.closest<HTMLElement>("#IcyLyricsPage")
      ?.classList.remove("FullscreenLyricsPresenting");
    if (!deferRemeasure) {
      this._virtualizer?._willUpdate();
      requestAnimationFrame(() => this._remeasureVisible());
    }
  }

  public deferPresentationRemeasure(): void {
    this._presentationRemeasureGate.defer();
  ...
  public flushDeferredPresentationRemeasure(): boolean {
    return this._presentationRemeasureGate.flush(() => {
      const v = this._virtualizer;
      const scrollEl = this._scrollEl;
      if (!v || !scrollEl) return;

      const width = scrollEl.clientWidth;
      const height = scrollEl.clientHeight;
      if (width > 0) this._containerWidth = width;
      this._containerHeight = height;
      if (this._spacer) this._spacer.style.height = `${height / 2}px`;
  ...
  public setPresentationIndices(indices: readonly number[]): void {
    this.setPresentationMode(true);
    const stage = this._presentationStage;
    if (!stage) return;

    const next = new Set(indices.filter((index) => index >= 0 && index < this._allElements.length));
    if (
      next.size === this._presentationIndices.size &&
      [...next].every((index) => this._presentationIndices.has(index))
    ) {
      return;
    }

    for (const index of this._presentationIndices) {
      if (next.has(index)) continue;
      const element = this._allElements[index];
      const wrapper = this._getOrCreateWrapper(index);
      ...
      if (wrapper.isConnected) this._virtualizer?.measureElement(wrapper);
    }

    // Append in document order. appendChild also reorders elements that were
    // already in the stage, which makes background-vocal stacking stable.
    [...next]
      .sort((a, b) => a - b)
      .forEach((index) => {
        const element = this._allElements[index];
        ...
        const measurement = this._virtualizer?.measurementsCache[index] as
          | { start: number }
          | undefined;
        if (!this._mountedIndices.has(index) && this._virtualContainer) {
          wrapper.style.transform = `translateY(${Math.round(measurement?.start ?? 0)}px)`;
          this._virtualContainer.appendChild(wrapper);
          this._mountedIndices.add(index);
        }
        this._resizeObserver?.unobserve(wrapper);
        element.dataset.fullscreenFocusIndex = String(index);
        carrier.classList.add("FullscreenLyricsFocusCarrier");
        stage.appendChild(carrier);
      });

    this._presentationIndices = next;
  }

  public isPresentationMode(): boolean {
    return this._presentationStage?.isConnected === true;
  }

  private _restorePresentationElements(): void {
    for (const index of this._presentationIndices) {
      const element = this._allElements[index];
      if (!element) continue;
      delete element.dataset.fullscreenFocusIndex;
    ...
    this._presentationIndices.clear();
  }

  /**
   * Commit the virtual-list destination synchronously while the lyric carriers
   * are still pinned in the focus stage. The normal async TanStack/RO pass is
   * deliberately deferred during a view FLIP; without this small synchronous
   * alignment, the carriers animate toward stale pre-focus wrapper positions
   * and jump when that deferred pass finally runs.
   */
  public preparePresentationExit(index: number, padding = 30): void {
    const v = this._virtualizer;
    const scrollEl = this._scrollEl;
    if (!v || !scrollEl || !Number.isInteger(index) || index < 0) return;

    this._presentationRemeasureGate.cancel();
    this.scrollToIndex(index, "center", true, padding);
    ...
    // Presentation wrappers retain their last measured sizes. Their absolute
    // starts are sufficient to establish exact mixed-view FLIP destinations;
    // expensive size reconciliation remains deferred until the animation ends.
    for (const presentationIndex of this._presentationIndices) {
      const measurement = v.measurementsCache[presentationIndex] as { start: number } | undefined;
      const wrapper = this._wrappers[presentationIndex];
      if (wrapper && measurement) {
        wrapper.style.transform = `translateY(${Math.round(measurement.start)}px)`;
      }
    }
  ...
  init(scrollEl: HTMLElement, virtualContainer: HTMLElement, lineElements: HTMLElement[]): void {
    ...
    this._carriers = Array.from({ length: lineElements.length }, () => null);
    ...
    this._resizeObserver = this._maid!.Give(
      new ResizeObserver((entries) => {
        ...
        for (const entry of entries) {
          ...
          if (this._presentationIndices.has(Number(indexAttr))) continue;
          v.measureElement(el);
          changed = true;
        }
  ...
  private _getOrCreateCarrier(index: number): HTMLElement {
    let carrier = this._carriers[index];
    if (!carrier) {
      carrier = document.createElement("div");
      carrier.className = "IcyLyricsLineCarrier";
      carrier.dataset.lyricsCarrierIndex = String(index);
      carrier.style.position = "relative";
      carrier.style.width = "100%";
      this._carriers[index] = carrier;
    }
    return carrier;
  }
  ...
  private _doOnVirtualizerChange(v: Virtualizer<HTMLElement, HTMLElement>): void {
    ...
    // Focus-stage items remain pinned with their previous valid measurements;
    // their live children are temporarily outside these wrappers.
    const nextVisible = new Set([...items.map((i) => i.index), ...this._presentationIndices]);
    ...
    for (const item of items) {
      ...
      if (this._presentationIndices.has(item.index)) {
  ...
  private _scrollToIndexWithRetry(
    ...
    if (retry < LyricsVirtualizer._MAX_SCROLL_RETRIES) {
      this._scrollVerifyRAF = requestAnimationFrame(() => {
        this._scrollVerifyRAF = null;
        if (this._presentationRemeasureGate.isPending) {
          this._setConverging(false);
          return;
        }
        if (this._virtualizer !== v) {
...
export function setLyricsPresentationMode(active: boolean, deferRemeasure = false): void {
  lyricsVirtualizer.setPresentationMode(active, deferRemeasure);
}

export function flushDeferredLyricsPresentationRemeasure(): boolean {
  return lyricsVirtualizer.flushDeferredPresentationRemeasure();
}

export function deferLyricsPresentationRemeasure(): void {
  lyricsVirtualizer.deferPresentationRemeasure();
}

export function setLyricsPresentationIndices(indices: readonly number[]): void {
  lyricsVirtualizer.setPresentationIndices(indices);
}

export function prepareLyricsPresentationExit(index: number, padding = 30): void {
  lyricsVirtualizer.preparePresentationExit(index, padding);
}

export function getLyricsLineCarrier(element: HTMLElement): HTMLElement | null {
  return element.closest<HTMLElement>(".IcyLyricsLineCarrier");
}

export function isLyricsPresentationMode(): boolean {
  return lyricsVirtualizer.isPresentationMode();
}
