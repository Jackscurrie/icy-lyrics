...
function renderCardShell(npv: HTMLElement): boolean {
  ...
  el.id = "IcyLyricsNPVCard";
  el.innerHTML = `
        <div class="CardHeader">
            <span class="CardTitle">Lyrics</span>
            <div class="CardControls">
                <button type="button" id="NPVCardExpand" class="CardControl">${Icons.CinemaView}</button>
                <button type="button" id="NPVCardMaximize" class="CardControl">${Icons.Maximize}</button>
                <button type="button" id="NPVCardToggle" class="CardControl">${Icons.Collapse}</button>
            </div>
        </div>
  ...
  if (expand) {
    ...
    setTooltip(expand, "Open Icy Lyrics", "expand-tip");
  }
...
async function reconcile(): Promise<void> {
  ...
  if (desired === "ACTIVE" && !cardOwnsPage && cardBodyEl) {
    ...
    try {
      ...
      if (!PageView.IsOpened) cardOwnsPage = false;
    } catch (err) {
      cardOwnsPage = false;
      throw err;
    }
    ...
    try {
      ...
      cardOwnsPage = PageView.IsOpened;
      throw err;
    }
    refreshCardUI();
  } else {
