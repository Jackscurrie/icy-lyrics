...
const OpenPopupLyricsFlow = async () => {
  ...
  // Copy the main Icy Lyrics style element.
  // Find any <style> element in the DOM that includes '#IcyLyricsPage' in its textContent
  // Find all <style> elements in the DOM that include '#IcyLyricsPage' in their textContent
  const icyLyricsStyleElement = document.querySelector("#ilstyles");
  let icyLyricsStyleContent: string | null = null;

  if (icyLyricsStyleElement) {
    if (icyLyricsStyleElement.tagName.toLowerCase() === "link") {
      // @ts-ignore
      const href = icyLyricsStyleElement.getAttribute("href");
      if (href) {
        try {
          const res = await fetch(href);
          if (res.ok) {
            icyLyricsStyleContent = await res.text();
          }
        } catch {
          icyLyricsStyleContent = null;
        }
      }
    } else if (icyLyricsStyleElement.tagName.toLowerCase() === "style") {
      icyLyricsStyleContent = icyLyricsStyleElement.textContent;
    }
  }

  if (icyLyricsStyleContent) {
    const newStyleElement = document.createElement("style");
    newStyleElement.textContent = icyLyricsStyleContent;
    currentPipWindow.document.head.appendChild(newStyleElement);
  }

  // Additionally, copy the runtime animation styles injected by Icy Lyrics.
  const additionalStyling = document.getElementById("icyLyrics-additionalStyling");
  if (additionalStyling) {
    const newAdditionalStyling = document.createElement("style");
    newAdditionalStyling.id = "icyLyrics-additionalStyling";
  ...
  currentPipWindow.document.body.innerHTML = `<div class="app-drag-region"></div><div class="icy-pip-wrapper"></div>`;

  const pipWrapper = currentPipWindow.document.body.querySelector(".icy-pip-wrapper") as HTMLElement;
  ...
  pipPageHideHandler = () => {
    // The user closed the document-PiP window through its native chrome. Clean
    // up the shared page owner in the opener instead of reloading all of
    // Spotify, then let the NPV card reclaim the singleton page if eligible.
    const closedWindow = currentPipWindow;
    ...
    void (async () => {
      try {
        ...
        if (currentPipWindow === closedWindow) currentPipWindow = null;
        IsPIP = false;
        RequestNPVCardEvaluate();
      }
    })();
