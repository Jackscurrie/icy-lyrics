...
function ApplyMarquee(baseWidth, elementWidth, name) {
    ...
    `;
    style.id = `icy-lyrics-marquee_${name}`;
...
function OpenNowBar(skipSaving: boolean = false) {
  ...
  if (icyLyricsPage) {
    icyLyricsPage.classList.remove("NowBarStatus__Closed");
    icyLyricsPage.classList.add("NowBarStatus__Open");
  }
    ...
    // Let's Apply more data into the fullscreen mode.
    {
        ...
        return {
          Apply: () => {
            if (IsCompactMode() || IsPIP || !$timelineOutsideMediaContent.get()) {
              // Timeline goes inside MediaContent with the other fullscreen
              // playback controls.
      ...
      const SetupVolumeControl = () => {
        ...
        const commit = (volume: number) => {
          ...
          try {
            ...
            console.error("Icy Lyrics: couldn't set the volume", err);
          }
        };
        ...
        const iconHandler = () => {
          try {
            ...
            console.error("Icy Lyrics: couldn't toggle mute", err);
            return;
          }
      ...
      Whentil.When(
        ...
        () => {
          ...
          // PageView owns .ViewControls as a stable ContentBox-level overlay.
          // Only refresh the fullscreen media controls in MediaContent.
          MediaContent.replaceChildren(fragment);
  ...
  /* const DragBox = Fullscreen.IsOpen
        ? document.querySelector(
              "#IcyLyricsPage .ContentBox .NowBar .Header .MediaBox .MediaContent"
          )
        : document.querySelector(
              "#IcyLyricsPage .ContentBox .NowBar .Header .MediaBox .MediaImageContainer"
          ); */

  /* {
        const dropZones = document.querySelectorAll(
            "#IcyLyricsPage .ContentBox .DropZone"
        );
        ...
        DragBox.addEventListener("dragstart", (e) => {
            ...
            document.querySelector("#IcyLyricsPage").classList.add("SomethingDragging");
        ...
        DragBox.addEventListener("dragend", () => {
            const missingLyrics = $currentLyricsData.get() === `NO_LYRICS:${SpotifyPlayer.GetSongId()}`;
            if (missingLyrics) return;
            document.querySelector("#IcyLyricsPage").classList.remove("SomethingDragging");
        ...
        dropZones.forEach((zone) => {
            ...
            zone.addEventListener("drop", (e) => {
                ...
                document.querySelector("#IcyLyricsPage").classList.remove("NowBarSide__Left");
                document.querySelector("#IcyLyricsPage").classList.remove("NowBarSide__Right");
                document.querySelector("#IcyLyricsPage").classList.add(`NowBarSide__${newClass.replace("Side", "")}`);
...
function CloseNowBar() {
  ...
  const icyLyricsPage = PageContainer;
  if (icyLyricsPage) {
    icyLyricsPage.classList.remove("NowBarStatus__Open");
    icyLyricsPage.classList.add("NowBarStatus__Closed");
  }
...
/* function UpdateNowBar(force = false) {
    const NowBar = document.querySelector("#IcyLyricsPage .ContentBox .NowBar");
    if (!NowBar) return;
...
function NowBar_SwapSides() {
  ...
  if (CurrentSide === "left") {
    ...
    icyLyricsPage.classList.remove("NowBarSide__Left");
    icyLyricsPage.classList.add("NowBarSide__Right");
  } else if (CurrentSide === "right") {
    $nowBarSide.set("left");
    NowBar.classList.remove("RightSide");
    NowBar.classList.add("LeftSide");
    icyLyricsPage.classList.remove("NowBarSide__Right");
    icyLyricsPage.classList.add("NowBarSide__Left");
  } else {
    $nowBarSide.set("right");
    NowBar.classList.remove("LeftSide");
    NowBar.classList.add("RightSide");
...
function DeregisterNowBarBtn() {
  /* const nowBarButton = document.querySelector(
        "#IcyLyricsPage .ContentBox .ViewControls #NowBarToggle"
    );
    nowBarButton?.remove(); */
  PageView.AppendViewControls(true);
}
