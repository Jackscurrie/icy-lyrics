...
export default function BackgroundSection({ query, sectionFilter }: Props) {
  ...
  const fullscreenBackgroundBlurEnabled = useStore($fullscreenBackgroundBlurEnabled);
  ...
  const r4 = matches(
    query,
    "Blur animated fullscreen background",
    "Blur the animated Kawarp background in Cinema and document fullscreen views."
  );
  ...
  return (
    <>
      ...
      {r4 && (
        <Row
          label="Blur animated fullscreen background"
          description="Blur only the animated page background in Cinema and fullscreen. Other glass and Now Playing effects are unchanged."
        >
          <Toggle
            checked={fullscreenBackgroundBlurEnabled}
            onChange={(value) => $fullscreenBackgroundBlurEnabled.set(value)}
          />
        </Row>
      )}
    </>
