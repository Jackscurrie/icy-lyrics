...
export function TrackRow({ uri, track, loading, isCurrentlyPlaying, onPlay, onDownload, onDelete }: TrackRowProps) {
  ...
  if (track == null && loading) {
    return (
      <div className="il-ldb-row il-ldb-row--skeleton">
        <div className="il-ldb-row__cover il-ldb-skeleton" />
        <div className="il-ldb-row__info">
          <div className="il-ldb-skeleton il-ldb-skeleton--title" />
          <div className="il-ldb-skeleton il-ldb-skeleton--artist" />
        </div>
        <div className="il-ldb-row__actions" />
      </div>
    );
  }
  ...
  return (
    <div className={`il-ldb-row${isCurrentlyPlaying ? " il-ldb-row--playing" : ""}`}>
      <img
        className="il-ldb-row__cover"
        src={coverUrl}
        alt=""
        width={40}
        height={40}
      ...
      />
      <div className="il-ldb-row__info">
        <div className="il-ldb-row__title-row">
          {isCurrentlyPlaying && (
            <span
              className="il-ldb-row__playing-dot"
              role="img"
              aria-label="Currently playing"
              title="Currently playing"
            />
          )}
          <span className="il-ldb-row__title" title={title}>{title}</span>
        </div>
        {artists && <span className="il-ldb-row__artists" title={artists}>{artists}</span>}
      </div>
      <div className="il-ldb-row__actions">
