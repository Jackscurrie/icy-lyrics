...
object TtmlParser {
  ...
  private const val XML_NS = XMLConstants.XML_NS_URI
  private val prohibitedDeclaration = Regex("<!\\s*(?:DOCTYPE|ENTITY)\\b", RegexOption.IGNORE_CASE)
  ...
  private fun parseXml(raw: String): Document {
    ...
    return try {
      val factory = DocumentBuilderFactory.newInstance().apply {
        ...
        // Android's built-in DocumentBuilderFactory only recognizes a small subset
        // of JAXP/Xerces features. Requiring any one of these flags makes every valid
        // TTML document fail before parsing on affected Android releases. Keep each
        // implementation-specific defense when supported, without making parser
        // compatibility depend on it. The declaration pre-scan and rejecting entity
        // resolver below are the platform-independent security boundary.
        ...
        // Android's XMLConstants omits these Java SE fields, but Xerces honors the
        // standard property URIs. Android's factory rejects unknown attributes.
        runCatching { setAttribute("http://javax.xml.XMLConstants/property/accessExternalDTD", "") }
        runCatching { setAttribute("http://javax.xml.XMLConstants/property/accessExternalSchema", "") }
      }
      val builder = factory.newDocumentBuilder().apply {
        setEntityResolver { _, _ -> throw UnsafeTtmlException("External entities are not allowed") }
      }
    ...
    companion object {
      fun from(root: Element, document: Document): TimingContext {
        val namespaceValues = root.attributes?.let { attributes ->
          (0 until attributes.length).map { attributes.item(it).nodeValue.lowercase() }
        }.orEmpty()
  ...
  private val OFFSET_TIME = Regex("([+]?(?:\\d+(?:\\.\\d*)?|\\.\\d+))(h|m|s|ms|f|t)", RegexOption.IGNORE_CASE)
  private val UNITLESS_TIME = Regex("[+]?(?:\\d+(?:\\.\\d*)?|\\.\\d+)")
