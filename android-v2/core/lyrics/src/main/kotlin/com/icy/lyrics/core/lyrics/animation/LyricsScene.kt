...
class LyricsSceneEngine {
  ...
  private fun String.emphasisUnits(): List<String> = buildList {
    var offset = 0
    while (offset < length) {
      val codePoint = Character.codePointAt(this@emphasisUnits, offset)
...
private fun Int.isInvisibleLyricCodePoint(): Boolean =
  Character.isWhitespace(this) || Character.isSpaceChar(this) || when (Character.getType(this)) {
    Character.CONTROL.toInt(), Character.FORMAT.toInt(), Character.LINE_SEPARATOR.toInt(),
    Character.PARAGRAPH_SEPARATOR.toInt(), Character.SPACE_SEPARATOR.toInt() -> true
    else -> false
  }
...
private fun Int.isPlaceholderDecorationCodePoint(): Boolean = when (Character.getType(this)) {
...
private fun Char.isPresentationWhitespace(): Boolean =
  isWhitespace() || Character.getType(this).let { type ->
