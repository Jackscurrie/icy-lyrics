...
object LyricsfileYamlParser {
  ...
  fun parse(
    ...
    val root = try {
      val settings = LoadSettings.builder()
        ...
        .setCodePointLimit(MAX_INPUT_CHARS)
        .build()
      val documents = Load(settings).loadAllFromString(rawYaml).take(2).toList()
  ...
  private fun Any.asLong(path: String): Long = when (this) {
    ...
    is BigInteger -> try {
      ...
      throw LyricsfileParseException("$path is outside the supported integer range")
    }
  ...
  private val VERSION_HEADER = Regex("(?m)^\\s*version\\s*:")
  private val METADATA_HEADER = Regex("(?m)^\\s*metadata\\s*:")
}
