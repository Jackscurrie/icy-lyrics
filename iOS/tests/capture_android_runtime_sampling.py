"""Capture real hardware RuntimeShader child sampling on an explicitly owned emulator."""
...
def main():
    ...
    output = root / "iOS/tests/results/android/runtime-child-sampling" / run_id
    ...
    log = run("shell", "am", "instrument", "-w", "-r", "-e", "class",
              "com.icy.lyrics.parity.RuntimeChildSamplingProbeTest", "-e", "samplingProbeRunId", run_id,
              "com.icy.lyrics.test/androidx.test.runner.AndroidJUnitRunner")
    print(log, flush=True)
    (output / "instrumentation.log").write_text(log, encoding="utf-8")
    run("pull", f"/sdcard/Android/data/com.icy.lyrics/files/runtime-child-sampling/{run_id}/.", output)
    if "OK (1 test)" not in log or "FAILURES" in log or "shortMsg=" in log:
        ...
        raise RuntimeError(f"Sampling probe failed; evidence preserved at {output}")
    report = json.loads((output / "report.json").read_text(encoding="utf-8"))
    assert {x["id"] for x in report["samples"]} == {"default-nearest", "linear-control"}
    assert report["widthPx"] == 128 and report["heightPx"] == 32
    for sample in report["samples"]:
        assert sample["canvasHardwareAccelerated"] and sample["paintFilterBitmap"]
    ...
    metadata["sourceSha256"] = {name: hashlib.sha256((root / name).read_bytes()).hexdigest() for name in [
        "iOS/tests/android/RuntimeChildSamplingProbeTest.kt", "iOS/tests/capture_android_runtime_sampling.py"]}
    (output / "provenance.json").write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
    print(f"Captured hardware Android sampling probes: {output}")
