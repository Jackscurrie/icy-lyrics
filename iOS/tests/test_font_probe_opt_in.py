...
def job_guard(name):
    ...
    return re.search(r"^    if: (.+)$", job, re.MULTILINE).group(1)


def evaluate_guard(guard, event, private, probe, only):
...
class FontProbeOptInTest(unittest.TestCase):
    def test_input_is_manual_boolean_false_by_default(self):
        ...
        for name in ("font_backend_probe", "font_backend_only"):
    ...
    def test_main_validation_evidence_precedes_optional_additional_capture(self):
        app = WORKFLOW.split("  ios-sideload:\n", 1)[1].split("  ios-font-backend-probe:\n", 1)[0]
