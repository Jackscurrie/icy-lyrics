...
**Status: source port implemented; the simulator suite and device packaging pass, and a validated unsigned IPA is available for owner testing.** See [validation status](docs/VALIDATION.md). Run [33949243061](https://github.com/Jackscurrie/icy-lyrics/actions/runs/33949243061) passed the desktop, Android and iPhone jobs at revision `b66d7a8`; the locally recovered owner delivery includes the Spotify callback-routing fix and is labelled **“simulator-verified IPA; physical iPhone validation pending.”** The callback fix retains strict PKCE state validation, but a physical-device Spotify retry is still required. All 20 archived Android portrait and landscape scenarios still match the original pixel-for-pixel after extraction. The iPhone captures do not yet match those Android references, so Android-identical appearance and physical-iPhone behavior remain acceptance requirements.
