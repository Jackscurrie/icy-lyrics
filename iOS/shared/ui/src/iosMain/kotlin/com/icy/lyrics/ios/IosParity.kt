...
import com.icy.lyrics.ui.ProvideAndroidFontScaling
...
fun createIcyParityViewController(scenarioId: String, onFrameDrawn: (String) -> Unit): UIViewController = ComposeUIViewController {
  ...
  ProvideAndroidFontScaling {
    CompositionLocalProvider(LocalIcyUiPlatform provides platform) {
      ...
      // Observe the same Density used by production iOS text. Physical density
      // and Dynamic Type scale remain native; SP/DP uses the Android app's curve.
      ...
      Box(Modifier.fillMaxSize().drawWithContent {
        ...
        onFrameDrawn("""{"contentWidthPx":${size.width.toInt()},"contentHeightPx":${size.height.toInt()},"composeDensity":${density.density},"fontScale":${density.fontScale},"fontScalingPolicy":"AndroidX ui-unit 1.11.0 SP/DP curve; native physical density and Dynamic Type scale","safeDrawingInsetsPx":[${insets.getLeft(density, direction)},${insets.getTop(density)},${insets.getRight(density, direction)},${insets.getBottom(density)}],"spToPx":{$fontSamples},"libraryDateText":$libraryDate}""")
      }) {
